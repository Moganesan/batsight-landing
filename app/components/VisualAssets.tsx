...
export const Scanline = () => (
  <div className="pointer-events-none absolute inset-0 z-20 bg-gradient-to-b from-transparent via-white/[0.02] to-transparent bg-[length:100%_4px] animate-scan" />
);

export const SectionLabel = ({ text }: { text: string }) => (
  <div className="flex items-center gap-3 mb-4">
    <div className="h-[1px] w-8 bg-gradient-to-r from-white/30 to-transparent"></div>
    <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-s9-text-muted">{text}</span>
  </div>
);
...
export const MapNode = ({ x, y, label, delay }: { x: number; y: number; label: string; delay: number }) => (
  <div className="absolute" style={{ left: `${x}%`, top: `${y}%` }}>
    <div className="relative">
      <div className="absolute -inset-4 border border-white/20 rounded-full opacity-0 animate-ping-slow" style={{ animationDelay: `${delay}s` }}></div>
      <div className="absolute -inset-8 border border-white/10 rounded-full opacity-0 animate-ping-slower" style={{ animationDelay: `${delay}s` }}></div>
      <div className="w-2 h-2 bg-white rounded-full relative z-10 shadow-[0_0_8px_rgba(255,255,255,0.3)]">
        <div className="absolute inset-0 bg-white animate-ping opacity-50"></div>
      </div>
      <div className="absolute top-2 left-2">
        <div className="h-[1px] w-8 bg-white/10 rotate-[-45deg] origin-top-left"></div>
        <div className="absolute top-[-20px] left-[20px] bg-s9-black border border-white/[0.08] px-1.5 py-0.5 whitespace-nowrap z-20">
          <span className="text-[8px] text-white/50 font-mono uppercase tracking-wider block">{label}</span>
        </div>
      </div>
    </div>
  </div>
